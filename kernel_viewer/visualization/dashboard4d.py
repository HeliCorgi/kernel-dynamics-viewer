...
def plot_dashboard4d(kernel4d, report=None, path=None):
    """4D summary: central z-slice at the latest clean time, row-normalized
    radial profile, shape/anisotropy curves, transport exponent."""
    rk = kernel4d.radial()
    ef = kernel4d.edge_fraction()
    clean = np.where(np.isfinite(ef) & (ef <= 0.3))[0]
    i_snap = int(clean[-1]) if len(clean) else len(kernel4d.t) - 1
    iw, iz = kernel4d.origin[0], kernel4d.origin[1]

    fig, ax = plt.subplots(2, 2, figsize=(11, 8.5))
    a = ax[0, 0]
    ext = [kernel4d.x[0], kernel4d.x[-1], kernel4d.y[0], kernel4d.y[-1]]
    im = a.imshow(kernel4d.K[i_snap, iw, iz], origin="lower", extent=ext, cmap="magma")
    a.set_title(f"K(x, y, z=0, w=0) at t = {kernel4d.t[i_snap]:g} (central slice, latest clean frame)")
    ...
    im = a.pcolormesh(rho, kernel4d.t, P / rowmax, shading="auto", cmap="magma")
    a.set_xlabel("r (hyperspherical shell)"); a.set_ylabel("t")
    a.set_title("radial profile P(r, t) / max_r (row-normalized)")
    plt.colorbar(im, ax=a)

    a = ax[1, 0]
    E, _ = ring_strength(rk); V = violation_amplitude(rk)
    FA = kernel4d.fractional_anisotropy(); Q4 = kernel4d.cubic_anisotropy()
    m = np.zeros(len(kernel4d.t), dtype=bool); m[clean] = True
    a.plot(kernel4d.t[m], E[m], "o-", ms=3, color="#c44", label="ring E(t)")
    a.plot(kernel4d.t[m], V[m], "s-", ms=3, color="#888", label="violation V(t)")
    a.plot(kernel4d.t[m], FA[m], "^-", ms=3, color="#36c", label="FA(t)")
    a.plot(kernel4d.t[m], Q4[m], "v-", ms=3, color="#2a7", label="Q4(t), r>=2")
    ...
    a = ax[1, 1]
    tm, sl = local_loglog_slope(kernel4d.t, kernel4d.sigma2())
    ...
    title = "transport exponent (full-4D moments)"
